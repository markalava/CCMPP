...
\item[Fertility Rates] Fertility rates in the first and last age groups (i.e., $\ltp{n}{F}{0}$ and $\ltp{n}{F}{\omega}$) are assumed to be zero for all projection steps:
  ...
  \end{equation}
  \item[Survival Proportions] The survival proportion for the first age group, $\ltp{n}{S}{0}[t,t+n]$, is the proportion of births during the projection step $[t,t+n]$ that survive to the end of the step, ie.., year $t+n$. This is commonly estimated from the life table as
    \begin{equation}
      \label{eq:defn-nS0}
